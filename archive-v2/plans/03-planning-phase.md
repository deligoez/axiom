...
Init Mode uses a **conversational approach** with 🔍 Analyzer Ace guiding the user through project setup. The UI is rendered as an **80% height modal** in the main layout, keeping the header and footer visible.
...
**Key Design Decision:** Init flow is ONLY for first-time projects. Existing projects (with `.chorus/` directory) skip directly to Implementation.
...
Step 1: Welcome Screen
    │
    ├─► [M] Meet the Team ──► Step 2a: Show Personas ──► Step 3
    │
...
│  │                        Welcome to CHORUS                                  │  │
│  │                                                                           │  │
│  │  Chorus is your AI-powered development team. We'll help you build        │  │
│  │  software with multiple specialized agents working in parallel.          │  │
│  │                                                                           │  │
│  │  ─────────────────────────────────────────────────────────────────────   │  │
│  │                                                                           │  │
│  │  What we'll do:                                                          │  │
│  │                                                                           │  │
│  │  1. 🔍 Ace will analyze your project structure                           │  │
│  │  2. Configure quality commands (tests, linting, etc.)                    │  │
│  │  3. Set up the .chorus/ directory with all agent configurations          │  │
│  │  4. You'll be ready to start planning!                                   │  │
│  │                                                                           │  │
│  │  ─────────────────────────────────────────────────────────────────────   │  │
│  │                                                                           │  │
│  │  [M] Meet the Team - See all 9 agent personas                            │  │
│  │  [Enter] Start initialization with Ace                                   │  │
│  │                                                                           │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                  │
├─────────────────────────────────────────────────────────────────────────────────┤
│ [M] Meet Team   [Enter] Start   [q] Quit   [?] Help                             │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### Step 2a: Meet the Team (Optional)

Shown when user presses `M` on Welcome Screen. Navigate with arrow keys, press `Enter` to continue.

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│ 😎 CHORUS                                                    Meet the Team 1/9 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  ┌───────────────────────────────────────────────────────────────────────────┐  │
│  │                                                                           │  │
...
│  │  ─────────────────────────────────────────────────────────────────────   │  │
│  │                                                                           │  │
│  │  Ace analyzes your codebase during initialization. Sharp and thorough,   │  │
│  │  Ace identifies your tech stack, test framework, and existing patterns.  │  │
│  │                                                                           │  │
│  │  Responsibilities:                                                        │  │
│  │  • Detect project type and structure                                     │  │
│  │  • Identify test frameworks and quality tools                            │  │
│  │  • Recommend quality commands                                            │  │
...
│                                                                                  │
├─────────────────────────────────────────────────────────────────────────────────┤
│ [←/→] Navigate personas   [Enter] Continue to initialization   [ESC] Back       │
└─────────────────────────────────────────────────────────────────────────────────┘
```

After viewing all 9 personas (Ace, Ed, Pat, Finn, Sam, Lou, Dan, Will, Carl), the final screen shows:

```
│  │           Now let's start the initialization with 🔍 Ace!                │  │
│  │                                                                           │  │
│  │                     [Enter] Start with Ace                                │  │
```

### Step 3: Ace Init Process (Conversational)

This is the main init screen - a chat interface with Ace analyzing the project.
...
├─────────────────────────────────────────────────────────────────────────────────┤
│ [Enter] Accept & Continue   [Type] Customize   [q] Quit                         │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
```
├─────────────────────────────────────────────────────────────────────────────────┤
│ > add knip command for dead code detection                                      │
├─────────────────────────────────────────────────────────────────────────────────┤
```

Ace responds conversationally:

```
│  │  Got it! I've added the knip command:                                     │  │
│  │                                                                           │  │
│  │  ┌─ Quality Commands (Updated) ───────────────────────────────────────┐  │  │
│  │  │ test:      npm run test:run                                        │  │  │
│  │  │ typecheck: npm run typecheck                                       │  │  │
│  │  │ lint:      npm run lint                                            │  │  │
│  │  │ knip:      npm run knip          ← NEW                             │  │  │
│  │  └────────────────────────────────────────────────────────────────────┘  │  │
│  │                                                                           │  │
│  │  Anything else, or ready to proceed?                                      │  │
```

### Step 4: Complete (Auto-Scaffold)

When user presses Enter to accept, Ace immediately creates the `.chorus/` directory - **no confirmation dialog needed**.
...
│  │  ─────────────────────────────────────────────────────────────────────   │  │
│  │                                                                           │  │
│  │  Next: 📊 Planner Pat will help you create tasks for your project.       │  │
│  │                                                                           │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────────────────────────┤
...
## Review Mode UI (Task Validation Phase)

Task validation before implementation.

> **Terminology Note:** This "Review Mode" is the **Task Validation Phase** that occurs after planning but before implementation. It validates task structure (dependencies, acceptance criteria, etc.). This is different from the `review` task status in [11-review-sprint.md](./11-review-sprint.md), which is for **Human Review of Completed Work** during implementation.
...
Init creates the complete `.chorus/` directory. All files are created even if empty.
...
│ ══════════════════════════════════════════════════════════════════
│
├── config.json              # Main config (Ace analysis result)
├── task-rules.md            # Task validation rules (agent-readable)
├── PATTERNS.md              # Cross-agent patterns (empty initially)
├── planning-state.json      # Current planning state
├── pending-patterns.json    # Later queue for pattern suggestions: []
├── session-log.jsonl        # Session event log (append-only)
...
│
├── templates/
│   └── scratchpad.md        # Template for .agent/scratchpad.md in worktrees
│
│ ══════════════════════════════════════════════════════════════════
...
│ ══════════════════════════════════════════════════════════════════
│
├── hooks/                   # Empty directory for user hooks
│
│ ══════════════════════════════════════════════════════════════════
│ RUNTIME STATE (gitignored)
│ ══════════════════════════════════════════════════════════════════
│
├── state/                   # XState persistence
...
│
│ ══════════════════════════════════════════════════════════════════
│ SHARED RULES (All agents inherit these)
│ ══════════════════════════════════════════════════════════════════
│
├── rules/
│   ├── signal-types.md      # Signal format: <chorus>TYPE:payload</chorus>
...
│
│ ══════════════════════════════════════════════════════════════════
│ AGENTS (All 9 personas - same structure for each)
│ ══════════════════════════════════════════════════════════════════
│
└── agents/
    │
    ...
    │   ├── skills/          # Claude Code skills (empty initially)
    ...
    │   └── metrics.json
    │
    ├── will/                # 👁️ Watcher Will - Health Monitor
    │   ├── prompt.md
    │   ├── rules.md
    │   ├── skills/
    │   ├── logs/
...
**File Formats:**
- **JSON:** config.json, planning-state.json, spec-progress.json, metrics.json, feedback/*.json (structured data)
- **JSONL:** tasks.jsonl, session-log.jsonl, state/events.jsonl, logs/*.jsonl (append-only logs)
- **Markdown:** task-rules.md, PATTERNS.md, rules/*.md, agents/*/prompt.md, specs/*.md, templates/*.md (agent-readable)
- **Directory:** checkpoints/{timestamp}/ contains snapshot data for rollback

**Notes:**
- All 9 agents have the **same symmetric structure** (6 files each)
- Per-agent learnings: `.chorus/agents/{name}/learnings.md`
- Shared learnings go to Claude's native: `.claude/rules/learnings.md`

### Init Scaffolding (What Gets Created)

When Init Mode completes, the following files and directories are created automatically. **No confirmation dialog is shown** - Ace creates everything immediately upon user acceptance.

**Root Files (committed to git):**

| File | Init Default Content |
|------|---------------------|
| `config.json` | Ace analysis result (mode, maxAgents, qualityCommands) |
...
| `pending-patterns.json` | `[]` (empty array) |
| `session-log.jsonl` | Empty file |
| `tasks.jsonl` | Empty file |
| `templates/scratchpad.md` | Master plan scratchpad template |
| `specs/spec-progress.json` | `{}` (empty object) |

**Shared Rules (committed to git):**

| File | Init Default Content |
|------|---------------------|
| `rules/signal-types.md` | Signal protocol: `<chorus>TYPE:payload</chorus>` |
| `rules/learning-format.md` | Learning scope prefixes |
| `rules/commit-format.md` | Commit format with task ID and agent |
| `rules/completion-protocol.md` | Quality checks protocol |

**Per-Agent Files (committed to git, for all 9 personas):**

| File | Init Default Content |
|------|---------------------|
| `agents/{name}/prompt.md` | Default persona prompt |
...
**Directories (gitignored - runtime data):**

| Directory | Purpose |
|-----------|---------|
| `state/` | XState persistence (snapshot.json, events.jsonl) |
| `agents/{name}/logs/` | Per-task execution logs |
| `agents/{name}/skills/` | Persona-specific Claude Code skills |
| `metrics/` | Carl's metrics output |
| `checkpoints/` | Rollback checkpoint storage |
| `specs/archive/` | Completed specs archive |
| `feedback/` | Task review feedback |
| `hooks/` | User-defined hooks (empty initially) |

**Gitignore additions:**

```gitignore
# Chorus runtime (added to .gitignore by Ace)
.chorus/state/
.chorus/agents/*/logs/
.chorus/agents/*/skills/
.chorus/metrics/
.chorus/checkpoints/
