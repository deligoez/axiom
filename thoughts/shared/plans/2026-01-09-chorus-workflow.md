...
Phase 4.1: Core Loop (MVP)           ~15 tests
Phase 4.2: Parallelism               ~20 tests
Phase 4.3: Merge Service             ~25 tests
Phase 4.4: Autopilot Mode            ~20 tests
Phase 4.5: Memory & Learning         ~15 tests
Phase 4.6: Polish                    ~20 tests
─────────────────────────────────────────────
Total:                               ~115 tests
Current:                              176 tests
Target:                              ~290 tests
...
### Phase 4.1: Core Loop (MVP)

```
┌─────────────────────────────────────────────────────────────────┐
│ Phase 4.1: Core Loop - MINIMUM VIABLE ORCHESTRATION              │
│ Est: ~15 tests                                                   │
└─────────────────────────────────────────────────────────────────┘

Goal: "I can assign a task and watch agent complete it"

Features:
- [ ] Semi-auto mode only
- [ ] Task selection in TUI (arrow keys + Enter)
- [ ] Single agent spawn with task prompt
- [ ] Worktree creation (git worktree add)
- [ ] Agent output streaming to TUI
- [ ] Signal detection (<chorus>COMPLETE</chorus>)
...
Goal: "3 agents working in parallel"

Features:
- [ ] Multiple agents (maxParallel config)
- [ ] Tiling agent view (dynamic grid)
- [ ] Agent slot management
- [ ] Worktree per agent
- [ ] Independent output streaming
- [ ] Grid configuration (g key)

Files:
- src/components/AgentTilingView.tsx     (NEW)
- src/hooks/useAgentGrid.ts              (NEW)
- tests/components/AgentTilingView.test.ts (NEW)
```

### Phase 4.3: Merge Service

```
┌─────────────────────────────────────────────────────────────────┐
│ Phase 4.3: Background Merge Service                              │
│ Est: ~25 tests                                                   │
└─────────────────────────────────────────────────────────────────┘

Goal: "Completed tasks auto-merge with intelligent conflict resolution"

Features:
- [ ] MergeQueue (FIFO + priority + dependencies)
- [ ] Background merge worker
- [ ] Conflict classification (simple/medium/complex)
- [ ] Auto-resolve for simple conflicts
- [ ] Rebase-retry for medium conflicts
- [ ] Resolver agent for complex conflicts
- [ ] Human fallback for unresolvable
- [ ] TUI integration (StatusBar + M key)

Files:
- src/services/MergeService.ts           (NEW)
- src/services/ConflictResolver.ts       (NEW)
- src/stores/mergeStore.ts               (NEW)
- src/components/MergePanel.tsx          (NEW)
- tests/services/MergeService.test.ts    (NEW)
- tests/services/ConflictResolver.test.ts (NEW)
```

### Phase 4.4: Autopilot Mode

```
┌─────────────────────────────────────────────────────────────────┐
│ Phase 4.4: Autopilot Mode (Ralph Loop)                           │
│ Est: ~20 tests                                                   │
└─────────────────────────────────────────────────────────────────┘

Goal: "Start and walk away"

Features:
- [ ] Ralph loop (task→complete→next)
- [ ] Iteration counter + limit
- [ ] Completion detection (signal + tests)
- [ ] Dependency cascade (task done → dependents unblock)
- [ ] Safeguards (max iterations, timeout, stuck detection)
- [ ] Mode toggle (m key)
- [ ] Pause/resume (Spacebar)

Files:
- src/services/RalphLoop.ts              (NEW)
- src/services/CompletionChecker.ts      (NEW)
- src/hooks/useAutopilot.ts              (NEW)
- tests/services/RalphLoop.test.ts       (NEW)
```
...
Features:
- [ ] Scratchpad per agent session
- [ ] Learning extraction on task completion
- [ ] learnings.md management
- [ ] Prompt injection for non-Claude agents
- [ ] View learnings (L key)
- [ ] Cross-agent knowledge sharing

Files:
- src/services/MemoryService.ts          (NEW)
- src/services/LearningExtractor.ts      (NEW)
- src/components/LearningsPanel.tsx      (NEW)
- tests/services/MemoryService.test.ts   (NEW)
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Phase 4.6: Polish & Production Ready                             │
│ Est: ~20 tests                                                   │
└─────────────────────────────────────────────────────────────────┘
...
Files:
- src/services/HooksService.ts           (NEW)
- src/services/RollbackService.ts        (NEW)
- src/services/InitService.ts            (NEW)
- src/cli.ts                             (UPDATE)
- tests/services/HooksService.test.ts    (NEW)
- tests/services/RollbackService.test.ts (NEW)
```

### Implementation Order

```
Phase 4.1 (MVP) ← START HERE
    │
    ├── Single agent, single task
    ├── Foundation for everything else
    └── Can demo basic orchestration
    │
    ▼
Phase 4.2 (Parallelism)
    │
    ...
    │
    ▼
Phase 4.3 (Merge Service) ← CRITICAL FOR PARALLEL
    │
